...
function Footer() {
    return (
        <div className="w-full flex gap-10 h-screen bg-[#f1f1f1] p-8 font-['Founder_Grotesk_X-Condensed']">
            <div className="w-1/2 h-full flex flex-col justify-between">
                <div className="heading">
                    <h1 className="text-[8vw] font-semibold uppercase leading-none -mb-15">Building</h1>
                    <h1 className="text-[8vw] font-semibold uppercase leading-none -mb-15">Tech!</h1>
                </div>
            ...
            <div className="w-1/2 overflow-hidden cursor-pointer">
                ...
                <div className="mt-10 block text-xl">S:</div>
                <div className="details font-['Neue_Montreal_Regular'] mt-7">
                    {["Instagram", "Facebook", "Twitter", "LinkedIn"].map((item, index) => (
                        <div key={index} className="w-1/2 block text-xl font-light">
                            {item}
                        </div>
                    ))}
                </div>
                <div className="mt-10 block text-xl">L:</div>
                <div className="details font-['Neue_Montreal_Regular'] mt-7">
                    {["202-1965 W 4th Ave"].map((item, index) => (
                        <div key={index} className="w-1/2 block text-xl font-light">
                            {item}
                        </div>
                    ))}
                    {["30 Chukarina St", "Lviv, Ukraine"].map((item, index) => (
                        <div key={index} className="w-1/2 block text-xl underline-offset-1 mt-1 font-light">
                            {item}
                        </div>
                    ))}
                </div>
